...
| --- | --- | --- | --- |
| README stats | Third-party action in a read-only job, followed by validation | `automation/github-stats` | `assets/github-stats.svg` |
| Contribution grid | Repository script with response fixtures and validation | `automation/contribution-grid` | `contribution-grid.svg` |

The publication step starts from a clean checkout, validates the generated output, rejects any unexpected repository path, commits only the allow-listed asset, and creates or updates a pull request. Reused automation branches are updated with an explicit force-with-lease against the observed remote SHA.
