...
- Keep `Connect` as one compact centered row of the seven colored Shields links, using the existing transparent spacer plus left-aligned images so GitHub does not stack or spread them apart. Preserve the historical badge colors and make the LinkedIn badge read `LinkedIn`. Do not replace it with a table, generic `<kbd>` boxes, a monochrome treatment, split left/right floats, or a multi-row layout.
