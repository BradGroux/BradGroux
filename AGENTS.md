...
- Keep `Connect` as one centered row of the seven colored `for-the-badge` Shields links, using the existing transparent spacer plus left-aligned images so GitHub does not stack them. Do not replace it with a table, generic `<kbd>` boxes, or a multi-row layout.
